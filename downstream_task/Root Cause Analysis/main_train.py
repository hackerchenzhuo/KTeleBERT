...
if __name__ == '__main__':
    ...
    parser.add_argument('--step', default='train', choices=['train', 'test'], type=str)
    ...
    parser.add_argument('--pt_emb_path', default='./pretrain/yht_serialize_withAttribute_emb_Pre_train_0926_v41_seed42_epoch12_cls.pt')
    parser.add_argument('--rule_path', default='./rule/rule.txt')
    parser.add_argument('--task_name', default='test')
    parser.add_argument('--use_rule', default='no')
    ...
    train_g_list = [FaultGraph(pkg) for pkg in get_pkg_list(args, args.train_data_path, rule_set)]
    valid_g_list = [FaultGraph(pkg) for pkg in get_pkg_list(args, args.valid_data_path, rule_set)]

    trainer = Trainer(args, train_g_list, valid_g_list, valid_g_list)

    if args.step == 'train':
        trainer.train()
    elif args.step == 'test':
        trainer.before_test_load()
