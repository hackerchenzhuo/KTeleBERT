...
class Trainer(object):
    def __init__(self, args):
        ...
        self.graph_classifier = NodeClassifier(512, 128, 2).to(self.args.gpu)
    ...
    def train(self):
        ...
        for e in range(1, self.num_epoch + 1):
            ...
            for batch in self.train_loader:
                ...
                batch_ins_logit = self.graph_classifier(ent_emb)
                batch_ins_label = torch.cat([torch.LongTensor(batch[2]) for batch in batch]).to(self.args.gpu)

                batch_pred = torch.max(batch_ins_logit, dim=-1)[1]
                train_pred.append(batch_pred)
                train_label.append(batch_ins_label)

                batch_loss = self.loss_func(batch_ins_logit, batch_ins_label)
            ...
            acc = sum(train_pred == train_label) / train_label.shape[0]
    ...
    def eval(self, data_loader):
        ...
        for batch in data_loader:
            ...
            batch_pred = torch.max(batch_ins_logit, dim=-1)[1]
            eval_pred.append(batch_pred)
            eval_label.append(batch_ins_label)

        eval_pred = torch.cat(eval_pred)
        eval_label = torch.cat(eval_label)

        acc = sum(eval_pred == eval_label) / eval_label.shape[0]
